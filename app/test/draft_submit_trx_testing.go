...
// ShowDraftSubmitTrxBadRequest runs the method Show of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowDraftSubmitTrxBadRequest(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.DraftSubmitTrxController) (http.ResponseWriter, error) {
	...
	rw := httptest.NewRecorder()
	u := &url.URL{
		Path: fmt.Sprintf("/sao/v1/draft-submit-transaction/"),
	}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		panic("invalid test " + err.Error()) // bug
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "DraftSubmitTrxTest"), rw, req, prms)
	showCtx, _err := app.NewShowDraftSubmitTrxContext(goaCtx, req, service)
	if _err != nil {
		e, ok := _err.(goa.ServiceError)
		if !ok {
			panic("invalid test data " + _err.Error()) // bug
...
// ShowDraftSubmitTrxNotImplemented runs the method Show of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
